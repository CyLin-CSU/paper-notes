...
最终"句子"为 \( X \in \mathbb{R}^{N \times l} \)（N 个 token，维度 l）。
...
- 采用 **Linformer 式低秩线性注意力**：引入两个可学习的低秩投影矩阵 \( E \in \mathbb{R}^{N 	imes d} \)、\( F \in \mathbb{R}^{d 	imes N} \)（\( d \ll N \)），注意力图从 N×N 压缩到 N×d，时间/空间复杂度对 N 均为线性 `O(Nkd)`；
...
- 同一 BIOT 编码器分别编码 S 和 S̃；S̃ 侧接两层 predictor：\( Z = \mathrm{BIOT}(S) \)，\( \tilde{Z} = \mathrm{predictor}(\mathrm{BIOT}(\tilde{S})) \)；
...
4. 频率信息以 FFT 能量特征形式进入 segment embedding——证明频域特征对 CHB-MIT、IIIC 与可穿戴 HAR 任务尤其有用；
...
!!! abstract "TL;DR"
    ...
    - 9 个数据集（EEG/ECG/HAR）上超过或持平绝大多数基线；带缺失设定下性能衰减最小
...
:material-arrow-left: [上一篇：TFM-Tokenizer](0918-tfm-tokenizer.md) ｜ :material-arrow-right: [下一篇：LaBraM](0918-labram.md) ｜ :material-vector-link: [Tokenization 演进](../../topics/tokenization.md) ｜ :material-chart-box: [战绩总表](../../comparison.md#跨论文-benchmark-战绩表)
